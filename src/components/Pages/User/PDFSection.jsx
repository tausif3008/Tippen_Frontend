...
const PDFSection = () => {
  ...
  const [useUrl, setuseUrl] = useState(URLS.GET_PDF_PENDING_URL);

  const selector = useSelector((state) => state.pdfFilesUser);

  useEffect(() => {
    if (!selector)
      dispatch(GetPDFFilesUserAction({ URL: URLS.GET_PDF_PENDING_URL }));
  }, []);
    ...
    if (selector) {
      ...
      for (let index = 0; index < res.length; index++) {
        ...
        newTalbleArray.push({
          ...
          //     maxLength={4}
          //     onChange={(value) => {
          //       polygonCount[res[index]["id"]] = value;
          ...
          //         setremarkObject(remarkObject);
          //       } else {
          //         remarkObject[[res[index]["id"]]] = newValue;
          //       }
  ...
  function getPageDetails(url) {
    setSelectedRowKeys([]);
    dispatch(userPDFPaginationAction({ URL: url }));
  }
  ...
  function dispatchPdf(files) {
    let url = URLS.UPLOAD_PDF_FILE_URL + setedStatus + "/";
  ...
  const pdfUploadingCompletedSelector = useSelector(
    (state) => state.qcFileUploadSuccessPDF
  );
  ...
  useEffect(() => {
    if (pdfUploadingCompletedSelector) {
      setUploadedFileStatus((s) => {
        return { totalPDFFiles: s.totalPDFFiles + 1 };
      });
      dispatch(setPdfFileUploadingFalse());
    }
  }, [pdfUploadingCompletedSelector]);
  ...
  const onFinish = (values) => {
    ...
    let baseUrl = URLS.GET_PDF_PENDING_URL;
