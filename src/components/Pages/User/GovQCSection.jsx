...
const GovQCSection = () => {
  ...
  const selector = useSelector((state) => state.govQCFilesUserMessage);

  // console.log(selector,useUrl, 'selector')

  useEffect(() => {
    dispatch(GetGovQCFilesUserAction({ URL: URLS.GET_GOVT_QC_PENDING_URL }));
  }, []);
  ...
  function getPageDetails(url) {
    setSelectedRowKeys([]);
    dispatch(userGovQCPaginationAction({ URL: url }));
  }
  ...
  function paginationPageChange(page, pageSize) {
    ...
    dispatch(GetGovQCFilesUserAction({ URL: url }));
  }

  const [setedStatus, setsetedStatus] = useState(null);
  const [file, setFile] = useState(null);

  const [pdFiles, setGovQCFiles] = useState({
    pdfFiles: [],
  });
  ...
  useEffect(() => {
    if (govQCUploadingCompletedSelector) {
      setUploadedFileStatus((s) => {
        return { totalPDFFiles: s.totalPDFFiles + 1 };
      });
      dispatch(setGovQCFileUploadingFalse());
    }
