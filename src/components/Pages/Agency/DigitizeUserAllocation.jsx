...
const DigitizeUserAllocation = ({ departmentId }) => {
  ...
  const selector = useSelector((state) => state.digitizeAgencyList);
  const userSelector = useSelector((state) => state.digitizeAgencyUsersList);

  useEffect(() => {
    if (!selector && !userSelector) {
      const userListUrl = URLS.DEPARTMENT_WISE_USER_LIST_URL + departmentId;
      dispatch(agencyDigitizeUseList({ URL: userListUrl }));
      dispatch(
        allocateToUserDigitizeAction({ URL: URLS.GET_ALLOCATE_TO_DIGITISE_URL })
      );
    }
  }, []);
  ...
  function getPageDetails(url) {
    setSelectedRowKeys([]);
    dispatch(DigitizePaginationAgencyAction({ URL: url }));
  }
  ...
  function allocateRectifyFilesToUser() {
    if (selectedUser) {
      let url = URLS.ALLOCATE_TO_AGENCY_TO_USER + selectedUser + "/";
      dispatch(
        allocateDigitizedFilesToUserAction({
  ...
  const userAllocateion = useSelector(
    (state) => state.digitizeAgencToUserMessage
  );
  ...
  useEffect(() => {
    if (userAllocateion) {
      setSelectedRowKeys([]);
      setcurrentPage(1);
      dispatch(allocateToUserDigitizeAction({ URL: useUrl }));
    }
  }, [userAllocateion]);
  ...
  const onFinish = (values) => {
    ...
    let baseUrl = "version_0/document/rectify-document-list/?";
...
export default DigitizeUserAllocation;
