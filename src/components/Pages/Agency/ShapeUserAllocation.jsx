...
const ShapeUserAllocation = ({ departmentId }) => {
  ...
  const selector = useSelector((state) => state.shapeAgencyList);
  const userSelector = useSelector((state) => state.shapeAgencyUsersList);

  useEffect(() => {
    if (!selector && !userSelector) {
      const userListUrl = URLS.DEPARTMENT_WISE_USER_LIST_URL + departmentId;
      dispatch(agencyShapeUseList({ URL: userListUrl }));
      dispatch(
        allocateToUserShapeAction({ URL: URLS.GET_ALLOCATE_TO_SHAPE_URL })
      );
    }
  }, []);
  ...
  //   dispatch(ShapePaginationAgencyAction({ URL: nextUrlNew }));
  // }

  function getPageDetails(url) {
    setSelectedRowKeys([]);
    dispatch(ShapePaginationAgencyAction({ URL: url }));
  }
  ...
  function allocateRectifyFilesToUser() {
    if (selectedUser) {
      let url = URLS.ALLOCATE_TO_SHAPE_AGENCY_TO_USER + selectedUser + "/";
      dispatch(
        allocateShapeFilesToUserAction({
  ...
  const userAllocateion = useSelector((state) => state.shapeAgencToUserMessage);

  useEffect(() => {
    if (userAllocateion) {
      setSelectedRowKeys([]);
      setcurrentPage(1);
      dispatch(allocateToUserShapeAction({ URL: useUrl }));
    }
  }, [userAllocateion]);
  ...
  const onFinish = (values) => {
    ...
    let baseUrl = "version_0/document/topology-complete-document-list/?";
...
export default ShapeUserAllocation;
